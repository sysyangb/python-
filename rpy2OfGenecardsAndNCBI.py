...
for line in open(_input,"r"):
    if re.search("^ensgId", line) or re.search("genesymbol",line):
        continue
...
if marker == "E":
    ...
    out.close()
if marker == "G":
    headers = ["species", "genesymbol" ,"ensgId", "entezeId", "hgncId", "strand", "length", "position", "summary"]
    out.write('\t'.join(headers) + '\n')
    for key in ensgList:
        temp = ["Homo_sapiens", key,"-", "-", "-", "-", "-", "-", "-"]
        out.write("\t".join(temp) + '\n')
...
for(i in 1:nrow(allgene)){
  if(allgene[i,3]=="-"){
    tryCatch({
      ...
      #ensg id
      allgene[i,3] <- as.character(xpathSApply(doc=html,path = "//div[@class='gc-subsection']/div[@class='gc-subsection-inner-wrap']/ul/li/a/text()",fun = xmlValue)[[3]])
      #entrze id 
      allgene[i,4] <- as.character(xpathSApply(doc=html,path = "//div[@class='gc-subsection']/div[@class='gc-subsection-inner-wrap']/ul/li/a/text()",fun = xmlValue)[[2]])
      #hgnc id 
      allgene[i,5] <- as.character(xpathSApply(doc=html,path = "//div[@class='gc-subsection']/div[@class='gc-subsection-inner-wrap']/ul/li/a/text()",fun = xmlValue)[[1]])
      #strand
      allgene[i,6] <- as.character(xpathSApply(doc = html,path = "//div[@class='gc-subsection']/div[@class='gc-subsection-inner-wrap']/div[@class='row']/div/dl/dd/text()",fun=xmlValue)[[3]])
      #length
      allgene[i,7] <- as.character(xpathSApply(doc = html,path = "//div[@class='gc-subsection']/div[@class='gc-subsection-inner-wrap']/div[@class='row']/div/dl/dd/text()",fun=xmlValue)[[2]])
      #position
      allgene[i,8] <- as.character(xpathSApply(doc = html,path = "//div[@class='gc-subsection']/div[@class='gc-subsection-inner-wrap']/div[@class='row']/div/dl/dt/text()",fun=xmlValue)[[1]])
      #summary
      allgene[i,9] <- as.character(gsub("\r\n","",xpathSApply(doc = html,path = "//*[@id='summaries']/div[1]",fun = xmlValue)))
    }, error = function(e){print(paste0("Error ",i," ",allgene[i,2]))}
    ) 
    Sys.sleep(runif(1,min=1,max=2))}
}
write.table(allgene[,c(-2,-9)],sep = "\t",file = "genecards.txt")
'''
rscriptE = '''
library(XML)
library(RCurl)
